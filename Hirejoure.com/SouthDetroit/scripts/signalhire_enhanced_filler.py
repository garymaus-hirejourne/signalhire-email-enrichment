...
# Cache for verified patterns, contacts, and phone numbers
_pattern_cache = {}
_verification_cache = {}
_signalhire_cache = {}
_phone_cache = {}
...
def get_comprehensive_phone_numbers(first_name: str, last_name: str, company_name: str, domain: str, linkedin_url: str = "") -> Tuple[List[str], Dict]:
    ...
    # Handle NaN/missing domain values
    if pd.isna(domain) or not domain or str(domain).lower() == 'nan':
        domain = ""
    if pd.isna(company_name) or not company_name:
        company_name = ""
    
    # Create cache key
    cache_key = f"{str(company_name).lower()}_{str(domain).lower()}"
...
def generate_and_verify_email_enhanced(first_name: str, last_name: str, company_name: str, domain: str, linkedin_url: str = "") -> tuple[str, bool, str, Dict, List[str]]:
    """Enhanced email generation with SignalHire + pattern detection + phone number extraction"""
    ...
    if best_email:
        ...
        return best_email, False, best_pattern, signalhire_result, phone_numbers
    
    return None, False, detected_pattern, signalhire_result, phone_numbers
...
def process_csv_file(input_file: str):
    ...
    for idx, row in df.iterrows():
        ...
        # Generate and verify email with SignalHire + patterns + phone numbers
        email, is_verified, method_used, signalhire_data, phone_numbers = generate_and_verify_email_enhanced(
            row['first_name'], 
            row['last_name'], 
            row['company'], 
            row['domain'],
